package net.minecraft.world;

import cpw.mods.fml.relauncher.Side;
import cpw.mods.fml.relauncher.SideOnly;
import net.minecraft.block.Block;
import net.minecraft.block.material.Material;
import net.minecraft.tileentity.TileEntity;
import net.minecraft.util.Vec3Pool;
import net.minecraft.world.biome.BiomeGenBase;
import net.minecraft.world.chunk.Chunk;

public class ChunkCache implements IBlockAccess
{
    private int field_72818_a;
    private int field_72816_b;
    private Chunk[][] field_72817_c;
    private boolean field_72814_d;
    private World field_72815_e;

    public ChunkCache(World p_i1964_1_, int p_i1964_2_, int p_i1964_3_, int p_i1964_4_, int p_i1964_5_, int p_i1964_6_, int p_i1964_7_, int p_i1964_8_)
    {
        this.field_72815_e = p_i1964_1_;
        this.field_72818_a = p_i1964_2_ - p_i1964_8_ >> 4;
        this.field_72816_b = p_i1964_4_ - p_i1964_8_ >> 4;
        int l1 = p_i1964_5_ + p_i1964_8_ >> 4;
        int i2 = p_i1964_7_ + p_i1964_8_ >> 4;
        this.field_72817_c = new Chunk[l1 - this.field_72818_a + 1][i2 - this.field_72816_b + 1];
        this.field_72814_d = true;
        int j2;
        int k2;
        Chunk chunk;

        for (j2 = this.field_72818_a; j2 <= l1; ++j2)
        {
            for (k2 = this.field_72816_b; k2 <= i2; ++k2)
            {
                chunk = p_i1964_1_.func_72964_e(j2, k2);

                if (chunk != null)
                {
                    this.field_72817_c[j2 - this.field_72818_a][k2 - this.field_72816_b] = chunk;
                }
            }
        }

        for (j2 = p_i1964_2_ >> 4; j2 <= p_i1964_5_ >> 4; ++j2)
        {
            for (k2 = p_i1964_4_ >> 4; k2 <= p_i1964_7_ >> 4; ++k2)
            {
                chunk = this.field_72817_c[j2 - this.field_72818_a][k2 - this.field_72816_b];

                if (chunk != null && !chunk.func_76606_c(p_i1964_3_, p_i1964_6_))
                {
                    this.field_72814_d = false;
                }
            }
        }
    }

    @SideOnly(Side.CLIENT)
    public boolean func_72806_N()
    {
        return this.field_72814_d;
    }

    public int func_72798_a(int p_72798_1_, int p_72798_2_, int p_72798_3_)
    {
        if (p_72798_2_ < 0)
        {
            return 0;
        }
        else if (p_72798_2_ >= 256)
        {
            return 0;
        }
        else
        {
            int l = (p_72798_1_ >> 4) - this.field_72818_a;
            int i1 = (p_72798_3_ >> 4) - this.field_72816_b;

            if (l >= 0 && l < this.field_72817_c.length && i1 >= 0 && i1 < this.field_72817_c[l].length)
            {
                Chunk chunk = this.field_72817_c[l][i1];
                return chunk == null ? 0 : chunk.func_76610_a(p_72798_1_ & 15, p_72798_2_, p_72798_3_ & 15);
            }
            else
            {
                return 0;
            }
        }
    }

    public TileEntity func_72796_p(int p_72796_1_, int p_72796_2_, int p_72796_3_)
    {
        int l = (p_72796_1_ >> 4) - this.field_72818_a;
        int i1 = (p_72796_3_ >> 4) - this.field_72816_b;
        return this.field_72817_c[l][i1].func_76597_e(p_72796_1_ & 15, p_72796_2_, p_72796_3_ & 15);
    }

    @SideOnly(Side.CLIENT)
    public float func_72808_j(int p_72808_1_, int p_72808_2_, int p_72808_3_, int p_72808_4_)
    {
        int i1 = this.func_72811_b(p_72808_1_, p_72808_2_, p_72808_3_);

        if (i1 < p_72808_4_)
        {
            i1 = p_72808_4_;
        }

        return this.field_72815_e.field_73011_w.field_76573_f[i1];
    }

    @SideOnly(Side.CLIENT)
    public int func_72802_i(int p_72802_1_, int p_72802_2_, int p_72802_3_, int p_72802_4_)
    {
        int i1 = this.func_72810_a(EnumSkyBlock.Sky, p_72802_1_, p_72802_2_, p_72802_3_);
        int j1 = this.func_72810_a(EnumSkyBlock.Block, p_72802_1_, p_72802_2_, p_72802_3_);

        if (j1 < p_72802_4_)
        {
            j1 = p_72802_4_;
        }

        return i1 << 20 | j1 << 4;
    }

    public int func_72805_g(int p_72805_1_, int p_72805_2_, int p_72805_3_)
    {
        if (p_72805_2_ < 0)
        {
            return 0;
        }
        else if (p_72805_2_ >= 256)
        {
            return 0;
        }
        else
        {
            int l = (p_72805_1_ >> 4) - this.field_72818_a;
            int i1 = (p_72805_3_ >> 4) - this.field_72816_b;
            return this.field_72817_c[l][i1].func_76628_c(p_72805_1_ & 15, p_72805_2_, p_72805_3_ & 15);
        }
    }

    @SideOnly(Side.CLIENT)
    public float func_72801_o(int p_72801_1_, int p_72801_2_, int p_72801_3_)
    {
        return this.field_72815_e.field_73011_w.field_76573_f[this.func_72811_b(p_72801_1_, p_72801_2_, p_72801_3_)];
    }

    @SideOnly(Side.CLIENT)
    public int func_72811_b(int p_72811_1_, int p_72811_2_, int p_72811_3_)
    {
        return this.func_72813_a(p_72811_1_, p_72811_2_, p_72811_3_, true);
    }

    @SideOnly(Side.CLIENT)
    public int func_72813_a(int p_72813_1_, int p_72813_2_, int p_72813_3_, boolean p_72813_4_)
    {
        if (p_72813_1_ >= -30000000 && p_72813_3_ >= -30000000 && p_72813_1_ < 30000000 && p_72813_3_ <= 30000000)
        {
            int l;
            int i1;

            if (p_72813_4_)
            {
                l = this.func_72798_a(p_72813_1_, p_72813_2_, p_72813_3_);

                if (l == Block.field_72079_ak.field_71990_ca || l == Block.field_72092_bO.field_71990_ca || l == Block.field_72050_aA.field_71990_ca || l == Block.field_72063_at.field_71990_ca || l == Block.field_72057_aH.field_71990_ca)
                {
                    i1 = this.func_72813_a(p_72813_1_, p_72813_2_ + 1, p_72813_3_, false);
                    int j1 = this.func_72813_a(p_72813_1_ + 1, p_72813_2_, p_72813_3_, false);
                    int k1 = this.func_72813_a(p_72813_1_ - 1, p_72813_2_, p_72813_3_, false);
                    int l1 = this.func_72813_a(p_72813_1_, p_72813_2_, p_72813_3_ + 1, false);
                    int i2 = this.func_72813_a(p_72813_1_, p_72813_2_, p_72813_3_ - 1, false);

                    if (j1 > i1)
                    {
                        i1 = j1;
                    }

                    if (k1 > i1)
                    {
                        i1 = k1;
                    }

                    if (l1 > i1)
                    {
                        i1 = l1;
                    }

                    if (i2 > i1)
                    {
                        i1 = i2;
                    }

                    return i1;
                }
            }

            if (p_72813_2_ < 0)
            {
                return 0;
            }
            else if (p_72813_2_ >= 256)
            {
                l = 15 - this.field_72815_e.field_73008_k;

                if (l < 0)
                {
                    l = 0;
                }

                return l;
            }
            else
            {
                l = (p_72813_1_ >> 4) - this.field_72818_a;
                i1 = (p_72813_3_ >> 4) - this.field_72816_b;
                return this.field_72817_c[l][i1].func_76629_c(p_72813_1_ & 15, p_72813_2_, p_72813_3_ & 15, this.field_72815_e.field_73008_k);
            }
        }
        else
        {
            return 15;
        }
    }

    public Material func_72803_f(int p_72803_1_, int p_72803_2_, int p_72803_3_)
    {
        int l = this.func_72798_a(p_72803_1_, p_72803_2_, p_72803_3_);
        return l == 0 ? Material.field_76249_a : Block.field_71973_m[l].field_72018_cp;
    }

    @SideOnly(Side.CLIENT)
    public BiomeGenBase func_72807_a(int p_72807_1_, int p_72807_2_)
    {
        return this.field_72815_e.func_72807_a(p_72807_1_, p_72807_2_);
    }

    @SideOnly(Side.CLIENT)
    public boolean func_72804_r(int p_72804_1_, int p_72804_2_, int p_72804_3_)
    {
        Block block = Block.field_71973_m[this.func_72798_a(p_72804_1_, p_72804_2_, p_72804_3_)];
        return block == null ? false : block.func_71926_d();
    }

    public boolean func_72809_s(int p_72809_1_, int p_72809_2_, int p_72809_3_)
    {
        Block block = Block.field_71973_m[this.func_72798_a(p_72809_1_, p_72809_2_, p_72809_3_)];
        return block == null ? false : block.field_72018_cp.func_76230_c() && block.func_71886_c();
    }

    @SideOnly(Side.CLIENT)
    public boolean func_72797_t(int p_72797_1_, int p_72797_2_, int p_72797_3_)
    {
        Block block = Block.field_71973_m[this.func_72798_a(p_72797_1_, p_72797_2_, p_72797_3_)];
        return this.field_72815_e.func_102026_a(block, this.func_72805_g(p_72797_1_, p_72797_2_, p_72797_3_));
    }

    public Vec3Pool func_82732_R()
    {
        return this.field_72815_e.func_82732_R();
    }

    @SideOnly(Side.CLIENT)
    public boolean func_72799_c(int p_72799_1_, int p_72799_2_, int p_72799_3_)
    {
        Block block = Block.field_71973_m[this.func_72798_a(p_72799_1_, p_72799_2_, p_72799_3_)];
        return block == null;
    }

    @SideOnly(Side.CLIENT)
    public int func_72810_a(EnumSkyBlock p_72810_1_, int p_72810_2_, int p_72810_3_, int p_72810_4_)
    {
        if (p_72810_3_ < 0)
        {
            p_72810_3_ = 0;
        }

        if (p_72810_3_ >= 256)
        {
            p_72810_3_ = 255;
        }

        if (p_72810_3_ >= 0 && p_72810_3_ < 256 && p_72810_2_ >= -30000000 && p_72810_4_ >= -30000000 && p_72810_2_ < 30000000 && p_72810_4_ <= 30000000)
        {
            if (p_72810_1_ == EnumSkyBlock.Sky && this.field_72815_e.field_73011_w.field_76576_e)
            {
                return 0;
            }
            else
            {
                int l;
                int i1;

                if (Block.field_71982_s[this.func_72798_a(p_72810_2_, p_72810_3_, p_72810_4_)])
                {
                    l = this.func_72812_b(p_72810_1_, p_72810_2_, p_72810_3_ + 1, p_72810_4_);
                    i1 = this.func_72812_b(p_72810_1_, p_72810_2_ + 1, p_72810_3_, p_72810_4_);
                    int j1 = this.func_72812_b(p_72810_1_, p_72810_2_ - 1, p_72810_3_, p_72810_4_);
                    int k1 = this.func_72812_b(p_72810_1_, p_72810_2_, p_72810_3_, p_72810_4_ + 1);
                    int l1 = this.func_72812_b(p_72810_1_, p_72810_2_, p_72810_3_, p_72810_4_ - 1);

                    if (i1 > l)
                    {
                        l = i1;
                    }

                    if (j1 > l)
                    {
                        l = j1;
                    }

                    if (k1 > l)
                    {
                        l = k1;
                    }

                    if (l1 > l)
                    {
                        l = l1;
                    }

                    return l;
                }
                else
                {
                    l = (p_72810_2_ >> 4) - this.field_72818_a;
                    i1 = (p_72810_4_ >> 4) - this.field_72816_b;
                    return this.field_72817_c[l][i1].func_76614_a(p_72810_1_, p_72810_2_ & 15, p_72810_3_, p_72810_4_ & 15);
                }
            }
        }
        else
        {
            return p_72810_1_.field_77198_c;
        }
    }

    @SideOnly(Side.CLIENT)
    public int func_72812_b(EnumSkyBlock p_72812_1_, int p_72812_2_, int p_72812_3_, int p_72812_4_)
    {
        if (p_72812_3_ < 0)
        {
            p_72812_3_ = 0;
        }

        if (p_72812_3_ >= 256)
        {
            p_72812_3_ = 255;
        }

        if (p_72812_3_ >= 0 && p_72812_3_ < 256 && p_72812_2_ >= -30000000 && p_72812_4_ >= -30000000 && p_72812_2_ < 30000000 && p_72812_4_ <= 30000000)
        {
            int l = (p_72812_2_ >> 4) - this.field_72818_a;
            int i1 = (p_72812_4_ >> 4) - this.field_72816_b;
            return this.field_72817_c[l][i1].func_76614_a(p_72812_1_, p_72812_2_ & 15, p_72812_3_, p_72812_4_ & 15);
        }
        else
        {
            return p_72812_1_.field_77198_c;
        }
    }

    @SideOnly(Side.CLIENT)
    public int func_72800_K()
    {
        return 256;
    }

    public int func_72879_k(int p_72879_1_, int p_72879_2_, int p_72879_3_, int p_72879_4_)
    {
        int i1 = this.func_72798_a(p_72879_1_, p_72879_2_, p_72879_3_);
        return i1 == 0 ? 0 : Block.field_71973_m[i1].func_71855_c(this, p_72879_1_, p_72879_2_, p_72879_3_, p_72879_4_);
    }
}
