package net.minecraft.world.gen.layer;

import net.minecraft.world.biome.BiomeGenBase;

public class GenLayerHills extends GenLayer
{
    public GenLayerHills(long p_i2126_1_, GenLayer p_i2126_3_)
    {
        super(p_i2126_1_);
        this.field_75909_a = p_i2126_3_;
    }

    public int[] func_75904_a(int p_75904_1_, int p_75904_2_, int p_75904_3_, int p_75904_4_)
    {
        int[] aint = this.field_75909_a.func_75904_a(p_75904_1_ - 1, p_75904_2_ - 1, p_75904_3_ + 2, p_75904_4_ + 2);
        int[] aint1 = IntCache.func_76445_a(p_75904_3_ * p_75904_4_);

        for (int i1 = 0; i1 < p_75904_4_; ++i1)
        {
            for (int j1 = 0; j1 < p_75904_3_; ++j1)
            {
                this.func_75903_a((long)(j1 + p_75904_1_), (long)(i1 + p_75904_2_));
                int k1 = aint[j1 + 1 + (i1 + 1) * (p_75904_3_ + 2)];

                if (this.func_75902_a(3) == 0)
                {
                    int l1 = k1;

                    if (k1 == BiomeGenBase.field_76769_d.field_76756_M)
                    {
                        l1 = BiomeGenBase.field_76786_s.field_76756_M;
                    }
                    else if (k1 == BiomeGenBase.field_76767_f.field_76756_M)
                    {
                        l1 = BiomeGenBase.field_76785_t.field_76756_M;
                    }
                    else if (k1 == BiomeGenBase.field_76768_g.field_76756_M)
                    {
                        l1 = BiomeGenBase.field_76784_u.field_76756_M;
                    }
                    else if (k1 == BiomeGenBase.field_76772_c.field_76756_M)
                    {
                        l1 = BiomeGenBase.field_76767_f.field_76756_M;
                    }
                    else if (k1 == BiomeGenBase.field_76774_n.field_76756_M)
                    {
                        l1 = BiomeGenBase.field_76775_o.field_76756_M;
                    }
                    else if (k1 == BiomeGenBase.field_76782_w.field_76756_M)
                    {
                        l1 = BiomeGenBase.field_76792_x.field_76756_M;
                    }

                    if (l1 == k1)
                    {
                        aint1[j1 + i1 * p_75904_3_] = k1;
                    }
                    else
                    {
                        int i2 = aint[j1 + 1 + (i1 + 1 - 1) * (p_75904_3_ + 2)];
                        int j2 = aint[j1 + 1 + 1 + (i1 + 1) * (p_75904_3_ + 2)];
                        int k2 = aint[j1 + 1 - 1 + (i1 + 1) * (p_75904_3_ + 2)];
                        int l2 = aint[j1 + 1 + (i1 + 1 + 1) * (p_75904_3_ + 2)];

                        if (i2 == k1 && j2 == k1 && k2 == k1 && l2 == k1)
                        {
                            aint1[j1 + i1 * p_75904_3_] = l1;
                        }
                        else
                        {
                            aint1[j1 + i1 * p_75904_3_] = k1;
                        }
                    }
                }
                else
                {
                    aint1[j1 + i1 * p_75904_3_] = k1;
                }
            }
        }

        return aint1;
    }
}
